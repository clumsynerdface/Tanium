...
.OUTPUTS
    <Drive:>\team\<filename>.ioc
.NOTES
    Runs from the team Scripting Server (X.us.ad.server.com) via scheduled task.
...
Try {
    ...
    $CyberPassReq.UserName = "dl-teamengineering@server.com"
    $CyberPassReq.AppID = "Prod"
    $CyberPassReq.Reason = "Get-ThreatStreamIOC.ps1"
}
Catch {
    Send-MailMessage -To $strEmailTo -SmtpServer smtp.server.com -From noreply@server.com -Subject "[ThreatStream IOC] - FAILURE - Unable to Obtain IOC feed for Tanium on $env:COMPUTERNAME" -Body "Failed to load CyberArk Credentials."
    Break
}
...
$strTSIType = "mal_md5"
$strEmailTo = "username@server.com"
...
"@
    ...
    Try {
        $xmlContents | Out-File \\server\team\ioc\$($strDate.Replace(":",".")).ioc -Encoding utf8 #Send to Dev
        $xmlContents | Out-File \\server\team\ioc\$($strDate.Replace(":",".")).ioc -Encoding utf8 #Send to Prod
...
If ($jsonResponse.count -eq 0) {
    Send-MailMessage -To $strEmailTo -SmtpServer smtp.server.com -From noreply@server.com -Subject "[ThreatStream IOC] - SUCCESS - IOC feed for Tanium on $env:COMPUTERNAME" -Body "No IOCs met our criteria for this run."
}

#Cleanup files older than 90 Days 

#Try {
#    Get-ChildItem -Path "\\X51472\team\ioc\","\\Xwvtan300\team\ioc\" -Recurse -File | Where-Object CreationTime -lt (Get-Date).AddDays(-90)  | Remove-Item
#    }
#catch {}